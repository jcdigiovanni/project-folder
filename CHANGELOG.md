...
### Changed
- **EC Combat Elixirs Data-Driven Refactor** — Extracted all hardcoded elixir static data (ingredient definitions, elixir names/effects, recipes, stash limits) from Dart classes into `ec_combat_elixirs.json`, matching the data-driven pattern established by `sororitas_trials.json`. Replaced 5 static classes (`StashLimits`, `ExoticIngredient`, `ArmyElixir`, `PersonalElixir`, `ElixirRecipes`) with JSON-loaded models (`ElixirLimits`, `IngredientDef`, `ElixirDef`, `ElixirSystemData`) and async cached loader (`loadElixirData()`). All 5 screen files updated to use data lookups. Backward-compatible with existing saved stash data (string keys unchanged).

### Added
- **EC Combat Elixirs** — Full inventory/crafting/equipping system for Emperor's Children Crusade mechanics. New `combat_elixirs.dart` data model with `CombatElixirsStash` (3 ingredient types: Common max 6, Rare max 4, Exotic max 2; 6 Army + 6 Personal Elixirs with max 3 doses each; recipe crafting with `ElixirRecipe`/`RecipeSlot`; consecutive-use blocking). Generic `factionDataJson` (HiveField 17) on Crusade for faction-specific data storage. `equippedElixirsJson` (HiveField 19) on Game for per-battle elixir snapshot. New `combat_elixirs_screen.dart` for stash management (ingredients, crafting, elixir doses, previous battle). OOB integration: stash button for EC crusades + auto-add Anfrak Silk on first unit. Play screen: equip elixirs step after agendas — army multi-select + personal per-CHARACTER with consecutive-use blocking and stash deduction. Active Game: collapsible equipped elixirs reference bar. Post-game: D6 ingredient rolls (Common D6+2 if won, Rare D6+1 if won). Propagated `factionDataJson` to all Crusade immutable constructors including provider `_copyWith`.
- **Victor Bonuses** — Post-game bonus selection for victorious players. 7 bonus types: Mark Additional Unit for Greatness (2 units get +3 XP), Free Supply Limit Increase (+200 on commit), Free Rearm and Resupply (deferred token), Free Repair and Recuperate (deferred token), Additional Requisition Point (+1 RP, cap 10), Free Battle Honour (deferred token), Free Weapon Enhancement (deferred token). Extensible `VictorBonusType` class with static constants and label/description maps. Deferred token architecture via `Crusade.pendingFreeRequisitions` (HiveField 16, `List<String>`) — tokens persist until redeemed on OOB or requisition screens. Victor Bonus section in post-game screen (victory only) with ChoiceChips and description text. Mark for Greatness refactored from single-select to multi-select (Set<String>) supporting 1 or 2 units. Free requisition tokens shown as "(FREE)" options on requisition screen with 0 RP cost. Free Battle Honour / Weapon Enhancement claimable from OOB unit details. `Game.victorBonus` (HiveField 18) records selection. Bonus logged in battle history event metadata.
- **Battle Honours as Currency** — Replaced `pendingRankUp` boolean with `availableBattleHonours` integer counter (HiveField 28) on UnitOrGroup. Supports multiple rank-up thresholds crossed in a single game (e.g., unit gaining enough XP to skip from Battle-ready to Battle-hardened awards 2 Battle Honours). New `rankIndex()` static helper for threshold counting. "Claim Battle Honour" button shows count when > 1. Added `floatingBattleHonours` (HiveField 15) to Crusade model for future mission reward support. Fixed `pendingRankUp` missing from `toJson()`/`fromJson()` — root cause of data loss during Google Drive backup/restore.
- **Trials of a Living Saint (FEA-017)** — Full SAINT POTENTIA / LIVING SAINT Crusade system for Adepta Sororitas. Designate one CHARACTER (non-Epic Hero) as SAINT POTENTIA on OOB screen with D6 roll or manual trial selection (6 trials from JSON data file). SAINT POTENTIA chip and trial info section (name, description, Saint Points progress bar) on unit card. Honor-system Saint Points +/- tracking on active game screen. Post-game commit applies per-game Saint Points to unit total, checks ascension threshold, shows ascension dialog with trial completion summary and Living Saint aura description. On ascension: replaces unit with LIVING SAINT inheriting all Battle Honours, Battle Scars, XP, and faction data; logs history event. Enforces only 1 SAINT POTENTIA at a time. Added `factionGameTracking` (HiveField 13) to UnitGameState for generic per-game faction tracking.
...
### Refactored
- **FactionCrusadeSystem abstraction (REFACTOR-001)** — Extracted all Sororitas-specific Trials of a Living Saint logic into a generic, reusable `FactionCrusadeSystem` class in `lib/models/faction_crusade_system.dart`. Configurable per-faction: system name, designation/ascension keywords, points label, icon, color, trial data asset path, action labels, and more. Registry pattern (`FactionCrusadeSystemRegistry`) for faction lookup. Moved `TrialDefinition` and `loadTrials()` from `oob_modify_screen.dart` into the new module. Renamed tally key `isLivingSaint` → `ProgressionKeys.isAscended`. Updated all 3 screens (OOB modify, active game, post-game) to use system properties for all labels, colors, event descriptions, and point calculations — zero hardcoded Sororitas text remains. Other factions can now add their own progression system by registering a `FactionCrusadeSystem` instance.
- **Shared code extraction (REFACTOR-002)** — Extracted repeated code from large screen files into shared utilities and widgets. New `GameLookups`/`CrusadeLookups` extension methods on `Game`/`Crusade` (`lib/utils/game_state_utils.dart`) replace ~11 `.where().firstOrNull` lookup patterns. New `GameUpdateMixin` (`lib/utils/game_update_mixin.dart`) eliminates duplicate `updateKills`/`updateDestroyed` methods across active game and post-game screens. Shared `TallyProgressBar` widget (`lib/widgets/tally_progress_bar.dart`) with optional milestones replaces two near-identical implementations. Shared `DetailRow` widget (`lib/widgets/detail_row.dart`) extracted from OOB screen. Consolidated duplicate `_loadBattleHonoursData()` in OOB screen via shared `loadBattleHonoursJsonData()` utility.
...
### Fixed
- **Post-Game Navigation Guard** — Navigating away from the post-game screen (via bottom nav, back button, or Android back) before committing results now shows a confirmation dialog warning that XP/tallies/scars haven't been applied. Added `isCommitted` (HiveField 17) to Game model to track commit state. Play screen now detects uncommitted games and prompts user to return to post-game review before starting a new battle. Prevents accidental data loss and OOA test bypass.
- **Battlefield Survivors XP Cap** — Fixed over-awarding XP for Battlefield Survivors at tier 2 (was 4 XP, should be 3 XP per rules). Added `maxXp: 3` cap to agenda data.
- **Agenda Multi-Select Bug** — Fixed unit selection dialog for multi-unit agendas (e.g., Battlefield Survivors `maxUnits: 3`). Previous implementation overwrote selections on each tap, only displayed first assigned unit, and froze the screen during selection. New dialog uses `StatefulBuilder` for multi-select toggle with Done/Cancel buttons, deferred state updates via `.then()` to prevent animation conflicts, and shows all assigned unit names with count (e.g., "Canoness, Retributors (2/3)"). Single-select agendas retain tap-to-select-and-close behavior.
