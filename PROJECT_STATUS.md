...
- Battlefield Survivors XP Fix: Added `maxXp: 3` cap — tier 2 was awarding 4 XP instead of 3 XP per rules.
- Battle Honours as Currency: Replaced `pendingRankUp` boolean with `availableBattleHonours` integer counter (HiveField 28). Supports multiple rank-ups per game via `rankIndex()` threshold counting. "Claim Battle Honour" shows count when > 1. Added `floatingBattleHonours` (HiveField 15) to Crusade for future mission rewards. Fixed `pendingRankUp` missing from JSON serialization (backup/restore data loss bug).
- Agenda Multi-Select Fix: Fixed unit selection dialog for multi-unit agendas (Battlefield Survivors etc.). Previous implementation overwrote selections, showed only first unit, and froze the screen. New `StatefulBuilder` dialog with multi-select toggle, deferred state updates, and full unit name display with count.
