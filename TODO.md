...
- **Feb 5**: ENH-012 (Settings backup/restore buttons stacked vertically for mobile), ENH-013 (Landing screen edge-to-edge with transparent status bar and notch-safe padding). Google Drive re-enabled — Android sign-in fixed (OAuth client IDs consolidated to Crusade Tracker GCP project, SHA-1 fingerprint registered, google-services.json with populated oauth_client), Web client ID updated to match. Verified working on Android and Chrome. Windows uses Chrome/web for Drive (google_sign_in plugin does not support Windows desktop).
