...
class RequisitionScreen extends ConsumerWidget {
          ...
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                // Faction-specific requisitions (conditional)
                ..._buildFactionRequisitions(context, ref, currentCrusade),
              ],
            ),
          ),
        ],
  ...
  void _showRearmAndResupplyModal(BuildContext context, WidgetRef ref, Crusade crusade) {
    showHonorSystemRequisitionModal(
      ...
      confirmTitle: 'Rearm and Resupply',
      confirmMessage: (name) => 'Spend 1 RP to swap wargear on $name?\n\nUpdate the unit\'s wargear on the tabletop as needed.',
      successMessage: (name) => 'Wargear swapped on $name (-1 RP)',
      eventDescription: (name) => 'Rearm and Resupply: Wargear swapped on $name',
      requisitionKey: 'rearm_and_resupply',
      rpCost: 1, icon: Icons.build, color: Colors.purple,
    );
  }
